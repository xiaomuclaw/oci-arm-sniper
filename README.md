...
实测发现 Oracle 对本租户的 launch 限流阈值约 **67 秒一次**。日志曾是完美交替：

```
no capacity → rate-limited → no capacity → rate-limited
```

即以 36 秒一次请求时，**每隔一次必被限流**。

关键点：**被限流的请求根本没碰到容量检查**，纯浪费。40 次尝试只换到 20 次
有效检查。所以放慢反而能提高有效检查数 —— 抢库存看的是有效检查次数，
不是尝试次数。
...
**被接受的请求数几乎恒定在 18-20 次/44 分钟**，无论请求多快。这是配额型
限流：Oracle 对本租户约每 140 秒才接受一次 launch，超出的一律 429。

结论：**有效检查次数被 Oracle 配额锁死，不是节奏能改善的。** 所以别再往
慢调（只会从 18 掉到 17），也别往快调（多出来的全是 429）。基准定在实测
均衡点 85 秒，`PACE` 仍会自调（限流 +15 秒 / 正常 -5 秒衰减，上限 180 秒），
以便 Oracle 改规则时自动跟上。

每轮结束打印 `useful capacity checks: N` 和 `final-pace`，这两个才是该看的。
